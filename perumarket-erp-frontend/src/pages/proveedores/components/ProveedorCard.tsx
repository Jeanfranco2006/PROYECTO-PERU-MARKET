...
export default function ProveedorCard({ proveedor, onEdit, onDelete, onViewProducts }: Props) {
    // Verificamos el estado (asegurando mayúsculas por si acaso)
    ...
    // Lógica para obtener el nombre correctamente (soporta backend Java camelCase o snake_case antiguo)
    // @ts-ignore - Ignoramos error de TS temporal si tu interfaz no tiene ambas propiedades
    const nombreEmpresa = proveedor.razonSocial || proveedor.razon_social || "Sin Razón Social";
    ...
    return (
        <div className="group relative flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl hover:shadow-slate-200/60 hover:-translate-y-1 transition-all duration-300 overflow-hidden">
            ...
            <div className="px-6 flex-1 flex flex-col relative">
                ...
                <div className="mb-5">
                    <h3 className="text-lg font-bold text-slate-800 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2" title={nombreEmpresa}>
                        {nombreEmpresa}
                    </h3>
                ...
                <div className="space-y-4 mb-4">
                    <InfoItem icon={<FaUser />} label="Contacto" value={proveedor.contacto} />
                    <div className="grid grid-cols-1 gap-3">
                        <InfoItem icon={<FaEnvelope />} label="Email Corporativo" value={proveedor.correo} isEmail />
                        <InfoItem icon={<FaPhone />} label="Teléfono" value={proveedor.telefono} mono />
                    </div>
                </div>
            ...
            {/* --- FOOTER DE ACCIONES --- */}
            <div className="px-4 py-3 bg-slate-50/80 border-t border-slate-100 flex items-center gap-2 backdrop-blur-sm mt-auto">
                <button
                    onClick={() => onViewProducts(proveedor)}
                    className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg transition-colors group/btn"
...
// Subcomponente InfoItem tipado correctamente
...
const InfoItem = ({ icon, label, value, isEmail, mono }: InfoItemProps) => (
  <div className="flex items-start gap-3 group/item">
    ...
    <div className="min-w-0 flex-1">
      ...
      <p className={`text-sm font-medium text-slate-700 leading-tight ${isEmail ? 'truncate hover:text-clip hover:whitespace-normal break-all' : 'truncate'} ${mono ? 'font-mono tracking-tight text-slate-600' : ''}`} title={value || ""}>
