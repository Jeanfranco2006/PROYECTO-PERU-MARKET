...
export default function ProveedorProductosModal({ isOpen, onClose, proveedor }: Props) {
  ...
  const cargarProductosProveedor = async () => {
    ...
    setLoading(true);
    try {
        // Ahora sí llamamos al endpoint correcto que creamos en el backend
        const res = await fetch(`${API_URL}/proveedores/${proveedor.id}/productos`);
  ...
  const handleVincular = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            cargarProductosProveedor(); // Recargar lista
            limpiarFormulario();
        } else {
  ...
  const handleDesvincular = async (idRelacion: number) => {
      ...
      try {
        const res = await fetch(`${API_URL}/proveedores/productos/${idRelacion}`, {
            method: 'DELETE'
        });
        if(res.ok) {
                      ...
                      <div key={prod.id} className="bg-white p-3 rounded-xl border border-slate-200 flex items-center justify-between gap-3 hover:border-indigo-200 transition-all">
                          <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                              ...
                              <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 shrink-0 overflow-hidden relative group">
                                  {prod.imagen ? (
                                      <img src={`${API_URL}${prod.imagen}`} alt={prod.nombre} className="w-full h-full object-cover"/>
                                  ) : (
